...
class SupConLoss(nn.Module):
    ...
    def forward(self, features, labels=None, mask=None):
        ...
        # set labels to [1,...] TODO which label should be used here?

        new_labels_1 = torch.ones(features.shape[0]).cuda()
        features = torch.cat([features.unsqueeze(dim=1), labels.unsqueeze(dim=1)], dim=1)
        labels = new_labels_1
        ...
        features = features/ torch.linalg.vector_norm(features, dim=[3,4], keepdim=True) # ord=2 default
